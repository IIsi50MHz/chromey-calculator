	...
	var prepareInputStr = (function () {
		...
		function prepareInputStr(str) {
			...
			str = str.replace(/([0-9a-zA-Z])\s+([a-zA-Z][^][-]?[0-9]|[a-zA-Z'"])/g, "$1|$2").replace(/([0-9a-zA-Z])\s+([a-zA-Z][^][-]?[0-9]|[a-zA-Z'"])/g, "$1|$2");
				// replace / between units with "`" so we can make things like this work as expected: 5 ft/sec / 5 in/sec = 12;
				str = str.replace(/([a-zA-Z])\s*\/\s*([a-zA-Z])/g, "$1`$2").replace(/([a-zA-Z])\s*\/\s*([a-zA-Z])/g, "$1`$2");
				// replace / between units with powers with "`" so we can make things like this work as expected: 5 ft^-1/sec / 5 in^-1/sec = 12;
				//console.log(str);
				str = str.replace(/([a-zA-Z][\\][-]*[^\s\+\-\*\/\^\{]+)\s*\/\s*([a-zA-Z])/g, "$1`$2").replace(/([a-zA-Z][\\][-]*[^\s\+\-\*\/\^\{]*)\s*\/\s*([a-zA-Z])/g, "$1`$2");
				//console.log(str);
