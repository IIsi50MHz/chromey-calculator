(function ($) {
	$(function () {
		...
		if (localStorage.prevInputs) {
			prevInputs = JSON.parse(localStorage.prevInputs);
			historyIndex = prevInputs.length - 1;
		}
		...
		window.storeCalcInfo = function () {			
			// store results and inputs
			localStorage.calcResults = $calcResults[0].innerHTML;
			localStorage.prevInputs = JSON.stringify(prevInputs);
		...
		$(document).keydown(function (e) {
			...
			if (+e.which === 13) { // enter			
				...
				// keep history of input expression (accessed with arrow keys)
				prevInputs.push($calcInput.val());
				if (prevInputs.length > maxResults) {
					// only keep last maxResults inputs
					prevInputs = prevInputs.slice(-maxResults);
				}
				historyIndex = prevInputs.length - 1;
				// clear input area
				$calcInput.val("")
				// focus calc input area
				$calcInput.focus();
			} else if (+e.which === 38) { // up arrow
				if (historyIndex >= 0) {
					// go through history (back)
					$calcInput.val(prevInputs[historyIndex]);
					historyIndex--;
				} else {
					// clear input area
					$calcInput.val("");
				...
				if (historyIndex < prevInputs.length - 1) {
					// go through history (forward)
					historyIndex++;
					$calcInput.val(prevInputs[historyIndex]);
				} else {
					// clear input area
